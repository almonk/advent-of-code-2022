...
# Find the LCM of all the monkey's divisors
divisors = $game.monkeys.map do |monkey|
  monkey.test_config[:test].split("divisible by").last.strip.to_i
end

# Find the LCM of the divisors
$master_divisor = divisors.inject(:lcm).to_i
